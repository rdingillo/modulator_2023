...
if os.name == 'posix':
    ...
    json_source = Path('/home/rocco/PycharmProjects/optical-system-interface/resources/ssfm_test_configs/lumentum_modulator/spectral_information')

elif os.name == 'nt':
    root = Path('C:/Users/din81017/PycharmProjects/optical-wideband-iq-modulator')
    json_source = Path('C:/Users/din81017/PycharmProjects/optical-system-interface/resources/ssfm_test_configs' \
                       '/lumentum_modulator/spectral_information')
...
output_csv = folder_results/"out_tx_time.csv"
...
list_sign = [xi, yi, xq, yq]
file = open(xi, "r")
csv_reader = csv.reader(file, delimiter=',')
data = np.array(list(csv_reader)).astype(float)
mod_sign_df = pd.DataFrame(columns=['xi','yi','xq','yq'], index=range(len(data)))
...
lpf = pd.read_csv(input_file, usecols= ['Freq [GHz]','dB20(S21diff) norm2'], index_col=False)
...
griffin_df = pd.DataFrame(griffin_tx_time)
classical_df = pd.DataFrame(classical_tx_time)
output_df = pd.DataFrame({'InP': griffin_tx_time, 'Legacy LiNbO3': classical_tx_time})


output_df.to_csv(output_csv, index=False, header=True)

plot_constellation(classic_tx_const, 'Standard MZM Constellation at RX side')
plot_constellation(griffin_tx_const, 'InP MZM Constellation at RX side')
